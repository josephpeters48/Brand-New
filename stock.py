...
return_list = []
final_df = pd.DataFrame(columns=[ 'Ticker', 'Latest_Price', 'Score' ' PE_Ratio', 'PEG_Ratio', 'SMA_150', 'SMA_200'])
...
best_performers = pd.DataFrame(list(zip(tickers, return_list)), colums= ['Ticker', 'Returns Compared'])
...
for ticker in best_performers['Ticker']:
    try:
        ...
        condition_5 = peg_ration < 2

        if condition_1 and condition_2 and condition_3 and condition_4 and condition_5:
            final_df = final_df.append({'Ticker': ticker, 'Latest_Price': latest_price, 'Score': score, 'PE_Ratio': pe_ratio, 'PEG_Ratio': peg_ratio, 'SMA_150': moving_average_150,'SMA_200':moving_average_200, '52_Week_Low': low_52week, '52_Week_High': high_52week}, ignore_index=True)
